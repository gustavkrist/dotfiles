...
## Get Brightness
if [[ -f /home/gustav/.local/bin/brightness ]]; then
  brightness_exec="/home/gustav/.local/bin/brightness"
  BNESS="$($brightness_exec query)"
  PERC="$BNESS"
  BLIGHT=${PERC%.*}
elif [[ -f /bin/brightnessctl ]]; then
...
case $chosen in
    "$ICON_UP")
    if [[ -f /home/gustav/.local/bin/brightness ]]; then
      /home/gustav/.local/bin/brightness + 5 && $notify "Brightness Up $ICON_UP"
		elif [[ -f /bin/brightnessctl ]]; then
    ...
    "$ICON_DOWN")
    if [[ -f /home/gustav/.local/bin/brightness ]]; then
      /home/gustav/.local/bin/brightness - 5 && $notify "Brightness Down $ICON_DOWN"
		elif [[ -f /bin/brightnessctl ]]; then
    ...
    "$ICON_OPT")
    if [[ -f /home/gustav/.local/bin/brightness ]]; then
      /home/gustav/.local/bin/brightness toggle && $notify "Optimal Brightness $ICON_OPT"
		elif [[ -f /bin/brightnessctl ]]; then
