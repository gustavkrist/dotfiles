...
local logger = require("lazy.core.util")
...
function M.wrap(toggle)
  return setmetatable(toggle, {
    __call = function()
      ...
      if state then
        logger.info("Enabled " .. toggle.name, { title = toggle.name })
      else
        logger.warn("Disabled " .. toggle.name, { title = toggle.name })
      end
      return state
    end,
...
function M.wk(lhs, toggle)
  if not require("util.plugins").has("which-key.nvim") then
    return
  end
  ...
  set = function(state)
    if state then
      ...
      vim.api.nvim_create_autocmd("ExitPre", {
        once = true,
        group = vim.api.nvim_create_augroup("window_restore_max_exit_pre", { clear = true }),
